...
public class TrafficWorld extends World {
	...
	private int[] horizontalRoads = new int[RC_AMOUNT];
	private int[] verticalRoads = new int[RC_AMOUNT+2];
	Random rand = new Random();
	public TrafficWorld(){
		super(WIDTH, LENGTH, 1);
		for(int roadAmount = RC_AMOUNT-1; roadAmount >= 0; roadAmount--){
			int roadY = (((ROAD_GAP + ROAD_WIDTH) * roadAmount)) + (ROAD_WIDTH/2);
			horizontalRoads[roadAmount] = roadY;
			Road road = new Road();
			this.addObject(road, WIDTH/2, roadY);
		}
		for(int roadAmount = (RC_AMOUNT+1); roadAmount >= 0; roadAmount--){
			int roadX = (((VERT_ROAD_GAP + ROAD_WIDTH) * roadAmount)) + (ROAD_WIDTH/2);
			verticalRoads[roadAmount] = roadX;
		...
		for(int i = 0; i < horizontalRoads.length; i++){
			for(int j = 0; j < verticalRoads.length; j++){
				...
				this.addObject(intersection, verticalRoads[j], horizontalRoads[i]);
				intersection.addVertLights();
				intersection.addHoriLights();
			}
		}
